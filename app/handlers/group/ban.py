...
async def func_ban(_, message: Message):
    ...
    if not re_msg:
        await message.reply_text("I don't know who you are talking about! Reply the member whom you want to ban!\nE.g`/ban reason`")
        return
    ...
    if chat_admins.is_victim_admin or chat_admins.is_victim_owner:
        await message.reply_text("I'm not going to ban an admin! You must be kidding!")
        return
    ...
    if not is_silent:
        text = f"{victim.mention.HTML} has been banned from this chat." + (f"\nReason: {reason}" if reason else "")
