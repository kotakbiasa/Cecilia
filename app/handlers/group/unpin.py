...
async def func_unpin(_, message: Message):
    ...
    if not re_msg:
        await message.reply_text("I don't know which message to unpin! Reply the message to unpin!")
        return
    ...
    if not is_silent:
        await message.reply_text(f"<a href='{re_msg.link}'>Message</a> has been unpinned!")
